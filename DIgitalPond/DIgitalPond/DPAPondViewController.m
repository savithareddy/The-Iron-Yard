...
-(void) swipe: (UISwipeGestureRecognizer *)gesture //UDM
{
    NSLog(@"%@",gesture);
...
//    [self.view addSubview:presentingBGView];
    [self.view insertSubview:presentingBGView atIndex:0]; // for the bgColors to change  under the ripples
...
{
    NSMutableArray *otherColors = [colors mutableCopy]; // making an aarray off of colors array
    [otherColors removeObjectAtIndex:currentBackground];
    
    //    touchpad is 1 touch and hold option key is 2nd touch
    for (UITouch *touch in touches) {
        int random = arc4random_uniform([otherColors count]);
