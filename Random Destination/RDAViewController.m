...
-(void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations
{
    ...
    NSArray *venues = [RDAFourSquareRequests getVenuesWithLat:currentLocation.coordinate.latitude andLong:currentLocation.coordinate.longitude];
//    NSLog(@"%@",venues);
    [self createMapAnnotationWithVenues:venues andLocation:currentLocation.coordinate];
...
//}
    ...
    CLLocationCoordinate2D centerCoordinate = CLLocationCoordinate2DMake(centerLat, centerLong);
      MKCoordinateRegion region = MKCoordinateRegionMake(centerCoordinate, MKCoordinateSpanMake((maxLat-minLat)+(maxLat-minLat)/5, (maxLong-minLong)+(maxLong-minLong)/5));
